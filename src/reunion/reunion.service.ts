...
export class ReunionService {
    ...
    async crearReunion(usuarioId: number, data: any): Promise<reunion> {
        ...
        try {
            ...
            diagramas.contenido = initialData;
    ...
    async obtenerReunionesDeUsuario(idUsuario: number): Promise<any> {
        ...
        SELECT reunion.id, reunion.*, diagrama.titulo AS diagrama_titulo, diagrama.descripcion AS diagrama_descripcion
        FROM reunion
        INNER JOIN diagrama ON reunion.diagramaid = diagrama.id
        WHERE reunion.usuarioid = $1
        GROUP BY reunion.id, diagrama.titulo,diagrama.descripcion;
      `, [idUsuario]);
